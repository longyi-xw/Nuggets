...
掘金自动签到 签到后会获得一次免费抽奖机会，自动触发免费抽奖，自动触发沾喜气，// 游戏功能无效 自动获取游戏奖励。
执行结束发送邮件通知签到结果。

Add.新增成长活跃任务，每天10点左右的成长值，自己可根据情况，新增沸点评论，发布沸点等，封号概率大，谨慎开发！！！

Add.兑换商品概览，我要马克杯！！！，兑换签证待破解。
...
| key | value | 是否必填 |
| --- | ---| ---|
| COOKIE | 按照仓库中的cookie.js文件中的cookie格式配置 | 是 |
| EMAIL_USER | 发送邮件的邮箱地址，该邮箱需要开启 SMTP | 否，如不想邮件提醒，则可随意填一个值 |
| EMAIL_PASS | 该邮箱的 SMTP 密码 | 否，如不想邮件提醒，则可随意填一个值 |
| EMAIL_TO | 接收邮件的邮箱 | 否，如不想邮件提醒，则可随意填一个值 |

`注意：掘金的cookie会有有效期，当你发现某天早上6点半左右没有发送邮件的时候就去重新配置cookie，所以建议开启邮箱推送`
